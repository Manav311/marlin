...
void Temperature::mintemp_error(const heater_id_t heater_id) {
  ...
  //min temp delay custom code
if (heater_id == H_BED) {
  static bool bed_mintemp_protection_active = false;
  static millis_t bed_mintemp_reached_time = 0;

  if (degBed() >= BED_MINTEMP) {
    if (bed_mintemp_reached_time == 0)
      bed_mintemp_reached_time = millis();
  } else {
    bed_mintemp_reached_time = 0;
  }

  if (bed_mintemp_reached_time && (millis() - bed_mintemp_reached_time >= 180000UL))
    bed_mintemp_protection_active = true;

  if (!bed_mintemp_protection_active)
    return;
}
//
  ...
  if (degBed() > BED_MAXTEMP) maxtemp_error(H_BED);
    ...
    do {
      ...
      if (!bed_timed_out) {
       ...
       //uday 
   //automatic bed set 
    #define BED_AUTO_TRIGGER_TEMP   115.0f
  #define BED_AUTO_SETPOINT_TEMP  129.0f 
  static bool bed_auto_heat_triggered = false;

  if (!bed_auto_heat_triggered && degBed() >= BED_AUTO_TRIGGER_TEMP && !temp_bed.target) {
   thermalManager.setTargetBed(BED_AUTO_SETPOINT_TEMP);
    bed_auto_heat_triggered = true;
  }
