...
void prepare_line_to_destination() {
  ...
  void homeaxis(const AxisEnum axis) {
    ...
    if (bump) {
      ...
      //uday
#if defined(Z_AFTER_HOMING) && Z_AFTER_HOMING > 0
  if (axis == Z_AXIS) {
    SERIAL_ECHO_START();
    SERIAL_ECHOLNPGM("Moving Z to Z_AFTER_HOMING height");
    current_position[Z_AXIS] = Z_AFTER_HOMING;

    // Set feedrate to Z homing feedrate
    feedrate_mm_s = homing_feedrate(Z_AXIS) / 1.0f;  // mm/min → mm/s
